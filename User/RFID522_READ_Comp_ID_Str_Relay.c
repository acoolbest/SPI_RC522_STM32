/*
   读取卡号（ID），以及Block1的前8个字节的内容（Str），如两者都符合就触发继电器（Relay）。
   Block1的前8个字节的内容（Str），可以通过《RFID522_Serial_write_block.ino》写入。

 * 文 件 名：RFID522_READ_Comp_ID_Str_Relay.ino
 * 修 改 者：mc.six

 * 功能描述：Mifare1 寻卡→防冲突→选卡→读卡→触发继电器
 */
...
void loop()
{
	...
	if( id[0]==0xF1 && id[1]==0x9A && id[2]==0xD6 && id[3]==0x0E && str[0]=='a'&& str[1]=='u'&& str[2]=='g'&& str[3]=='u'&& str[4]=='s'&& str[5]=='t'&& str[6]=='0'&& str[7]=='1' ) //根据需要手动修改每个的内容
	{
	...
	else if(id[0]==0xCD && id[1]==0x83 && id[2]==0xFE && id[3]==0xC9)  //根据需要手动修改每个的内容
	{
